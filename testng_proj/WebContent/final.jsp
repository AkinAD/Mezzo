...
<body>
	...
	<c:choose>
       ...
       <DIV> <h1>Order Successfully Completed</h1></DIV> 
		<div class="checkmark">
    ...
    <svg class="checkmark__background" height="115" viewBox="0 0 120 115" width="120" xmlns="http://www.w3.org/2000/svg">
		...
		<c:otherwise>
      <div class="failMsgArea">
        <div class="failCaption">
          <h1>
            <IMG alt=""
              src="https://i.giphy.com/media/xT9Igpm06uM5OJ5lVS/giphy.webp"
              width="32" height="32" />Payment Failed
            <c:choose>
              <c:when test="${requestScope.failReason == 'expCard'}">(Expired Card)</c:when>
              <c:when test="${requestScope.failReason == 'intentional'}">(TAs performing occult ritual)</c:when>
              <c:when test="${requestScope.failReason == 'emptyCart'}">(Empty cart)</c:when>
            </c:choose>
          </h1>
          <c:if test="${empty requestScope.failReason}">
            <h2>TAs performing occult ritual to read our spaghetti code:</h2>
          </c:if>
        </div>
        <c:url var="virtualSelf"
          value="https://www.youtube.com/embed/HPc8QMycGno">
          <c:param name="autoplay">1</c:param>
          <c:param name="start">130</c:param>
          <c:param name="controls">0</c:param>
          <c:param name="loop">1</c:param>
          <c:param name="playlist">HPc8QMycGno</c:param>
        </c:url>
        <iframe src="${virtualSelf}" allow="autoplay"></iframe>
      </div>
    </c:otherwise>
	</c:choose>
